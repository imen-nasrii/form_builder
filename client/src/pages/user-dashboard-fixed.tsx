          ...
          <CardContent>
            {programs.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <p>No programs created yet. Create your first program to get started!</p>
              </div>
            ) : (
              <div className="space-y-4">
                {(programs as any[]).map((program: any) => {
                  const StatusIcon = getStatusIcon(program.status);
                  const progressValue = getProgressValue(program.status);
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            ...
                            <Badge className={`text-white ${getStatusColor(program.status)}`}>
                              {program.status.toUpperCase()}
                            </Badge>
                          </div>
                          
                          <div className="text-sm text-gray-600 mb-3">
                            <span className="font-medium">Menu ID:</span> {program.menuId}
                          </div>
                        ...
                        <div className="flex space-x-2 ml-4">
                          ...
                          {program.status === 'in_progress' && (
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => stopProgramMutation.mutate(program.id)}
